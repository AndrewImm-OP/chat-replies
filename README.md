...
Клиентам мод не обязателен. Отображение replies делает сервер обычными chat components, поэтому игроки без клиентского мода всё равно видят нормальный формат ответов.

Клиентский jar можно ставить, но сейчас он не требуется для подключения к серверу. После установки перезапусти сервер.
...
`/chatreply <messageId>` обычно не вводят руками. Minecraft click event запускает эту команду сам, когда игрок кликает по тексту сообщения. Это работает и у клиентов без установленного Chat Replies, потому что клик приходит как обычное серверное действие в chat component.
...
- Клиентская установка не обязательна. Сервер сам хранит выбранный reply и сам рассылает форматированный чат.
