...
Положи файл из `build/libs/` в папку `mods/` сервера и клиентов:

```text
build/libs/chat-replies-<version>.jar
```

Если мод стоит на сервере, клиентская часть обязательна. Игрок без Chat Replies будет отключён при входе с сообщением:

```text
На этом сервере требуется клиентский мод Chat Replies.
```

После установки перезапусти сервер и клиенты.
...
- Если мод установлен на сервере, он требует такой же мод на клиенте. Проверка сделана через Fabric custom payload channel.
